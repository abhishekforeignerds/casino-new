            ...
            <div className="main-content-container sm:ml-52">
                <div className="mx-auto py-6">
                    <p className="flex">
                        ...
                        <FiChevronRight size={24} />
                        <span className="text-red">Super Admin List</span>
                    </p>
                    ...
                    <Link className="text-right bg-red px-8 py-2 rounded-md text-white block max-w-max ml-auto mb-4" href={route('subadmin.create')}>
                        Create Users
                    </Link>
                    ...
                    <div className="bg-white shadow-sm sm:rounded-lg p-6 text-gray-900">
                        <div className="top-search-bar-box flex py-4 gap-2">
                            <h2 className="font-bold text-2xl">Super Admin List</h2>
                        ...
                        {userPermissions.includes('subadmin-table users') && (
                            <div className="overflow-x-auto mt-6">
                                {paginatedUsers.length > 0 ? (
                                                ...
                                                <tr key={user.id}>
                                                    ...
                                                    <td className="px-2 py-3 border-b text-sm relative">
                                                        {userPermissions.includes('view users') && (
                                                            <Link className="bg-red px-3 py-1 rounded-md text-white text-xs" href={route('subadmin.addfund', user.id)}>
                                                                Add Fund
                                                            </Link>
                                                        )}
                                                        ...
                                                        {openDropdown === user.email && (
                                                            <div className="absolute right-0 mt-2 bg-slate-200 shadow-md rounded-lg p-2" onMouseLeave={closeDropdown}>
                                                                <Link href={route('subadmin.edit', user.id)} className="block hover:underline">Edit</Link>
                                                                <Link href={route('subadmin.view', user.id)} className="block hover:underline">View</Link>
                                                                <Link href={route('subadmin.suspend', user.id)} className="block hover:underline">Suspend</Link>
                                                            </div>
                                                        )}
                                                    </td>
                                                </tr>
                                    ...
                                    <p className="text-sm italic text-gray-500">No Super Admin users found.</p>
                                )}
