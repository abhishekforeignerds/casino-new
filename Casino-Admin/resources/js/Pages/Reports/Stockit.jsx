...
export default function Turnover({ gameResults, claimsByUser, saleToUser, unClaimsByUser }) {
    ...
    // 1) Filter by selected game & date
    ...
    const totals = useMemo(() => {
        ...
        pageData.forEach(rec => {
            ...
            if (role === 'Retailer') {
                ...
                const gstPercent = parseFloat(rec.client.stockit?.gstin_number || 0);
                ...
                const retdiffC = saleC - pointsC;
                const gstR = parseFloat(rec.client.stockit?.gstin_number || 0);
    ...
    // 7) Group by client.stockit.id and sum
    const stockitRows = useMemo(() => {
        const map = {};
        pageData.forEach(rec => {
            const s = rec.client.stockit || { id: 'unknown', name: '— No Stockit —', gstin_number: 0 };
            if (!map[s.id]) {
                map[s.id] = {
                    stockit: s,
                    totalBet: 0,
            ...
            const grp = map[s.id];
            grp.totalBet += rec.bet;
            grp.totalWin += rec.win_value;
            grp.totalSale += rec.sale_point;
            grp.totalPoints += rec.client.points;
            grp.totalClaim += rec.claim_point;
            grp.totalUnclaim += rec.unclaim_point;
        });
        ...
        return Object.values(map).map(e => {
            ...
            if (role === 'Retailer') {
                ...
                const gstPercent = parseFloat(e.stockit.gstin_number || 0);
                ...
                const retdiff = e.totalSale - pts;
                const gstR = parseFloat(e.stockit.gstin_number || 0);
                const retC1 = retdiff * (gstR / 100);
                const retnet = retdiff - retC1;
                const stkP = parseFloat(e.stockit.gstin_number || 0);
                    ...
                    <div className="bg-white shadow-md rounded-lg p-6 overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                ...
                                {/* One row per Stockit */}
                                {stockitRows.map(r => (
                                    <tr key={r.stockit.id} className="hover:bg-gray-50 text-sm">
                                        <td className="p-2 border-b">–</td>
                                        <td className="p-2 border-b">{r.stockit.name}</td>
