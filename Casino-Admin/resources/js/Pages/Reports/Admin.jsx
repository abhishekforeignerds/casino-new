...
export default function Admin({ gameResults, claimsByUser, saleToUser }) {
    ...
    }, [selectedGameResults, claimsByUser, saleToUser]);
    ...
    const totals = useMemo(() => {
        ...
        pageData.forEach(rec => {
            ...
            const pointC = toPaisa(parseFloat(rec.claim_point + rec.client.points));
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                <tr className="bg-gray-100 font-semibold text-sm">
                                    ...
                                    <td className="p-2 border-b">{(totals.totalPoints - totals.totalClaim).toFixed(2)}</td>
                                ...
                                {pageData.length ? (
                                    pageData.map(rec => {
                                        const bet = parseFloat(rec.sale_point);
                                        const points = parseFloat(rec.claim_point + rec.client.points);
                                        let diff = 0;
                                        let commAmount = 0;
                                        let netamount = 0;
                                        ...
                                        if (userRoles[0] === 'Retailer') {
                                            ...
                                            const gstPercent = parseFloat(rec?.client?.retailer?.gstin_number) || 0;
                                            const retcommAmount = (retdiff * (gstPercent / 100)).toFixed(2);
                                        ...
                                        return (
                                            <tr key={rec.client.id} className="hover:bg-gray-50 text-sm">
                                                ...
                                                <td className="p-2 border-b">{rec.bet}</td>
                                                <td className="p-2 border-b">{rec.win_value}</td>
                                                <td className="p-2 border-b">{rec.sale_point}</td>
                                                <td className="p-2 border-b">{rec.claim_point + rec.client.points}</td>
