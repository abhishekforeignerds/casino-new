...
export default function ViewTickets({ tickets: initialTickets, user }) {
    const [tickets, setTickets] = useState(initialTickets);
    const [lastPrintedId, setLastPrintedId] = useState(initialTickets?.[0]?.id || 0);

    useEffect(() => {
        const interval = setInterval(async () => {
            try {
                const response = await axios.get('/api/tickets/latest');
                const latestTickets = response.data;

                if (latestTickets.length > 0) {
                    const latestTicket = latestTickets[0];

                    if (latestTicket.id !== lastPrintedId) {
                        setTickets([latestTicket]);
                        setLastPrintedId(latestTicket.id);

                        setTimeout(() => {
                            const element = document.getElementById(`ticket-${latestTicket.id}`);
                            if (!element) return;
